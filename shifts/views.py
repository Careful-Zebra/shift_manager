...
def schedule_view(request, week_type, role_type):
    #start w/ all shifts sorted chronologically, and prefetch signups and users to avoid n+1 queries in the template
    shifts = Shift.objects.all().order_by('start_time').prefetch_related('signups__user')
    target_datetime = timezone.make_aware(datetime.datetime(2026, 5, 10)) #start of finals week, change in future semesters
...
def claim_shift(request, shift_id):
    ...
    past_webpage = request.META.get('HTTP_REFERER') #get the page they came from to redirect back to it at the end

    # Define the timezone-aware cutoff date for finals week
    target_datetime = timezone.make_aware(datetime.datetime(2026, 5, 10))

    #wrap everything important in one transaction to avoid race
    with transaction.atomic():
        #immediately lock shift row
        locked_shift = Shift.objects.select_for_update().get(id=shift.id)

        #check existing signups
        existing_signup = Signup.objects.filter(user=user, shift=locked_shift).first()
        if existing_signup:
            existing_signup.delete()
            return redirect(past_webpage or 'home')
        
        #permission check
        if user.boa_level in [1, 2] and locked_shift.shift_type == 'OPS_LEAD':
            messages.error(request, "You do not have the BOA Level required for this shift.")
            return redirect(past_webpage or 'home')
        if user.boa_level == 3 and locked_shift.shift_type != 'OPS_LEAD':
            messages.error(request, "Please only request Ops lead shifts for now.")
            return redirect(past_webpage or 'home')
        ...
        user_finals_count = Signup.objects.filter(
            user=user, shift__start_time__gte=target_datetime
        ).count()

        user_rrr_count = Signup.objects.filter(
            user=user, shift__start_time__lt=target_datetime
        ).count()
        ...
        signups = list(
            Signup.objects.select_for_update().filter(shift=locked_shift)
        )
